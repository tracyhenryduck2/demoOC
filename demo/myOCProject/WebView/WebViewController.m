...
- (void)createWebView{
    ...
    WKWebView *webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 64, self.view.frame.size.width, self.view.frame.size.height - 64) configuration:_config];
    webView.navigationDelegate = self;
    webView.UIDelegate = self;
    
//    NSString *js = @"document.getElementsByTagName('h2')[0].innerText = '我是ios为h5注入的方法'";
//    WKUserScript *script = [[WKUserScript alloc] initWithSource:js injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:YES];
//    [_config.userContentController addUserScript:script];
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"indexold.html" ofType:nil];
...
-(void)dealloc{
    [_config.userContentController removeScriptMessageHandlerForName:@"show"];
}
@end
